...
def save_midi_file(prediction_output):
    offset = 0
    output_notes = []
    _debug_empty_patterns = 0

    # create note and chord objects based on the values generated by the model
    for pattern in prediction_output:
        num_nonzero_entries = np.count_nonzero(pattern)

        # pattern is a chord
        if num_nonzero_entries > 1:
            non_zero_indices = []
        ...
        # pattern is a note
            ...
            _debug_empty_patterns += 1

        # increase offset each iteration so that notes do not stack
        offset += 0.5

    print(
        f"DEBUG: {_debug_empty_patterns / len(prediction_output) * 100}% of empty patterns"
    )
